...
.DESCRIPTION
  Clean helper script to download the official python.org exe and run it silently.
  Supports dry-run and optional SHA256 verification. Does not auto-elevate.

.USAGE
  # Dry-run with version resolution
  .\install-python-official-new.ps1 -Version 3.11.7 -DryRun -VerboseLog

  # Actual install (run PowerShell as Administrator if using -InstallAllUsers)
  .\install-python-official-new.ps1 -Url "https://www.python.org/ftp/python/3.11.7/python-3.11.7-amd64.exe" -InstallAllUsers
#>
...
try {
    ...
    if ($Sha256) {
        Write-Log 'Verifying SHA256 checksum...' 'INFO'
        if (-not $DryRun) {
    ...
    if ($DryRun) {
        ...
        Write-Log "Running installer: $installer $argString" 'INFO'
    ...
    Write-Log 'Done. Open a new shell to pick up PATH if installed.' 'INFO'
